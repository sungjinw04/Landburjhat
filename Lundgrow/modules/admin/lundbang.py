...
from Lundgrow.database import blacklist_users_db
...
async def bang_user(client, message):
    ...
    try:
        ...
        existing = await blacklist_users_db.find_one({"user_id": user_id})
        if existing:
            await message.reply(f"{first_name} is already banged.")
            return

        # Insert the user_id into the blacklist_user collection
        await blacklist_users_db.insert_one({"user_id": user_id, "first_name": first_name})
...
async def unbang_user(client, message):
    ...
    try:
        ...
        await blacklist_users_db.delete_one({"user_id": user_id})
...
async def lund_bang_list(client, message):
    ...
    try:
        ...
        banned_users = await blacklist_users_db.find({}).to_list(length=100)
