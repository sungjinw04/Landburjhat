...
from Lundgrow.database import blacklist_chats_db
...
async def blacklist_chat(client, message):
    ...
    try:
        ...
        existing = await blacklist_chats_db.find_one({"chat_id": chat_id})
        if existing:
            await message.reply("This chat is already blacklisted.")
            return

        # Insert the chat_id into the blacklist_collection
        await blacklist_chats_db.insert_one({"chat_id": chat_id})
...
async def blacklist_chat_list(client, message):
    ...
    try:
        ...
        blacklisted_chats = await blacklist_chats_db.find({}).to_list(length=100)
